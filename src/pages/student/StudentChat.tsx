    ...
    const conversations = useMemo(() =>
        coaches
            .map(coach => {
                ...
                const lastAt = loadedLast?.sentAt ?? coach.lastMessageAt ?? null;
                const lastContent = loadedLast?.content ?? coach.lastMessageContent ?? '';
                const liveUnread = msgs.filter(m => m.senderId === coach.id && !m.isRead).length;
                const baseUnread = openedIds.has(coach.id) ? 0 : (coach.unreadCount ?? 0);
                const unread = coach.id === selectedId ? 0 : Math.max(liveUnread, baseUnread);
                return {
                    id: coach.id,
        ...
        [coaches, messages, myId, selectedId, openedIds]
    );
